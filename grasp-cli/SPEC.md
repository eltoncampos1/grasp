...
| **v1.2** ✅ | **Entregue 2026-09-22:** MCP server em Go (`/mcp`, streamable HTTP hand-rolled) — 22 tools: leitura (list_changes, search/get_function com callers+comments, find_paths, reload_index…), canvas via bus SSE → tab aplica e autosalva (set_cards/open/close/focus/set_view/highlight_card), comments (add/reply/resolve/publish). Chat panel conecta o agente automaticamente via `--mcp-config` inline + `mcp__grasp` no allowlist. Cards sem scrollbar interna (conteúdo completo; scroll só horizontal). Layout em camadas (Sugiyama simplificado): callers→callees, baricentro anti-cruzamento, fluxos em bandas, isolados em grid, chip `entry` |
| **v1.3** ✅ | **Entregue 2026-09-22:** multi-select (⌘click, shift+drag box, multi-drag), undo (⌘Z, 30 snapshots), drag-select de ranges de comment (publica como multi-line nativo do GitHub), envio explícito ao GitHub (composer/thread/"send review" com considerações finais), groups/frames (⌘G/⇧⌘G, título renomeável e arrastável, drop-into-frame, herança de grupo ao abrir callee, tools MCP group_cards/ungroup_cards/rename_group + `group` no set_cards), largura adaptativa de card, porta com auto-bump, call links no diff |
| **v1 restante** | re-anchoring de comments quando linhas se movem, entry points de framework, `web --watch` |
