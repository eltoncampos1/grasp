...
Everything Grasp reads from your configuration is optional, and belongs in
`config/dev.exs`:
...
- Drag a frame's title to move the whole group: every card inside travels together and keeps
  its place relative to the others. A click on the title, with no drag, renames the group.
