defmodule Grasp.Index.Heex do
  ...
  Three kinds of text are skipped, because none of them compiles to a component call:
  slot tags (`<:inner>`, which are not components), `<%!-- --%>` and `<!-- -->` comments,
  and `{...}` interpolation. Interpolation is skipped by counting brace depth, not by
  parsing Elixir: a brace inside a string inside an interpolation (`{"{"}`) is counted like
  any other, so an unbalanced one swallows the rest of the template. EEx expression tags
  (`<%= ... %>`) are not skipped, so a tag written inside a string in one is reported.
  """
  ...
  defp tag_site(binary, state) do
    case Regex.run(@tag, binary, capture: :all_but_first) do
      [prefix, name] ->
